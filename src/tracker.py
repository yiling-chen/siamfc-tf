...
def tracker_vot(handle, hp, run, design, final_score_sz, filename, image, templates_z, scores):
    ...
    target_w = min(selection.width, img.shape[1] - 1)
    target_h = min(selection.height, img.shape[0] - 1)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            handle.report(vot.Rectangle(pos_x-target_w/2, pos_y-target_h/2, target_w, target_h), 0.5)
